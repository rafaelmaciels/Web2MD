...
export function convertToMarkdown(
  ...
  try {
    ...
    console.warn('[Web2MD Mobile] GFM plugin warning:', err);
  }
  ...
  turndown.addRule('fencedCodeBlockWithLang', {
    ...
    replacement: (_content, node) => {
      ...
      const className = (codeNode && typeof codeNode.getAttribute === 'function' ? codeNode.getAttribute('class') : '') || '';
  ...
  if (extracted.selectionText && extracted.selectionText.trim()) {
    ...
    try {
      bodyMarkdown = turndown.turndown(extracted.contentHtml || extracted.textContent || '');
    } catch (e) {
      console.warn('[Web2MD Mobile] Turndown parse error, falling back to text:', e);
      bodyMarkdown = extracted.textContent || '';
    }
  }
